package com.lakala.test.jmeter.cheat.model;

import java.io.Serializable;

/**
 * ������Ϣ
 * @author jack.guo
 *
 */
public class LoanApply implements Serializable {

	private String manageradvise;
	 private String _platform;
	 private String certsecondimg;
	 private String partneraccountbank;
	 private String userid;
	 private String orderno;
	 private String certfirstimg;
	 private String partnerid;
	 private String schname;
	 private String username;
	 private String married;
	 private String schcity;
	 private String emergencymobile;//紧急联系人手机号
	 private String income;
	 private String merchanttel;
	 private String hostname;
	 private String highestlevel;
	 private String industry;
	 private String usermobile;//推荐人手机号
	 private String channelid;
	 private String businesstype;
	 private String applyamt;
	 private String partnercontantmobile;//合作商户的手机号和字段merchantmobile一样
	 private String partneraccounttype;
	 private String macodes;
	 private String ocrcertno;
	 private String entnature;
	 private String uuid;
	 private String companyno;
	 private String busid;
	 private String position;
	 private String contactmobile;//亲属或朋友手机号码
	 private String getcontacts;
	 private String biztypeenum;
	 private String certno;
	 private String modelno;
	 private String manames;
	 private String wechatno;
	 private String debitbank;
	 private String partnercontantemail;
	 private String branchid;
	 private String quarters;
	 private String token;
	 private String cooporgno;
	 private String creditcard;
	 private String credtype;
	 private String telecode;
	 private String apcrnames;
	 private String hometel;
	 private String giftid;
	 private String sid;
	 private String graddate;
	 private String systemcode;
	 private String curcode;
	 private String partnercontant;
	 private String merchantorderexptime;
	 private String gifttips;
	 private String giftfeecode;
	 private String companyname;
	 private String process;
	 private String userapcrcodes;
	 private String schdorm;
	 private String collecttime;
	 private String partneraddr;
	 private String merchantname;
	 private String productno;
	 private String ocrname;
	 private String fcatschname;
	 private String hostip;
	 private String contactrelation;
	 private String authpwd;
	 private String _macwifi;
	 private String partnercompany;
	 private String userapcrnames;
	 private String usercoremobile;//登录手机号
	 private String _deviceid; //设备号
	 private String authname;
	 private String lbsaddress;
	 private String registerorg;
	 private String giftfeetype;
	 private String request;
	 private String creditmode;
	 private String brithday;
	 private String fingerprinttab;
	 private String identitycode;
	 private String paymode;
	 private String smscode;
	 private String credit_productno;
	 private String gifttype;
	 private String cpcrcodes;
	 private String merchantordertime;
	 private String nation;
	 private String merchantid;
	 private String resitemno;
	 private String schyear;
	 private String userfullife;
	 private String partnername;
	 private String sessionid;
	 private String schcitycode;
	 private String email;
	 private String rpflag;
	 private String applicant;
	 private String biztype;
	 private String merchantno;
	 private String debitcard;//借记卡
	 private String merchantmobile;//合作商户的手机号和字段partnercontantmobile一样
	 private String channelenum;
	 private String purseid;
	 private String mobile;//发送短信手机号即进件手机号
	 private String ipv4;//IP地址
	 private String sex;//性别
	 private String consumtype;
	 private String department;
	 private String merchantorderno;
	 private String emergencyrelation;
	 private String partneraccounter;
	 private String apcrcodes;
	 private String recommend;
	 private String merchantregistertime;
	 private String giftno;
	 private String creditbank;
	 private String channo;
	 private String cpcrnames;
	 private String bizno;
	 private String schroll;
	 private String qq;
	 private String emergencyname;
	 private String _macbluetooth;
	 private String logno;
	 private String branchcomp;
	 private String termid;
	 private String companyaddress;
	 private String merchantaddress;
	 private String userno;
	 private String partneraccount;
	 private String mchno;
	 private String address;
	 private String contactname;
	 private String registered;
	 private String nowphoto;
	 private String consumapplyid;
	 private String tips;
	 private String companytel;
	 private String worktime;
	 private String channelcode;
	 private String channelsource;
	 private String authenticationflag;
	 private String channelmobile;//合作渠道用户手机号
	 private String lbs;//lbs
	 private String lbsx;//lbsx
	 private String lbsy;//lbsy
	 private String lbsanalyzed;
	 private String lbsprovince;
	 private String receivetimefmt;
	 private String collecttimefmt;
	 private String belongprovince;
	 private String receivetime;

	public String getManageradvise() {
		return manageradvise;
	}
	public void setManageradvise(String manageradvise) {
		this.manageradvise = manageradvise;
	}
	public String get_platform() {
		return _platform;
	}
	public void set_platform(String _platform) {
		this._platform = _platform;
	}
	public String getCertsecondimg() {
		return certsecondimg;
	}
	public void setCertsecondimg(String certsecondimg) {
		this.certsecondimg = certsecondimg;
	}
	public String getPartneraccountbank() {
		return partneraccountbank;
	}
	public void setPartneraccountbank(String partneraccountbank) {
		this.partneraccountbank = partneraccountbank;
	}
	public String getUserid() {
		return userid;
	}
	public void setUserid(String userid) {
		this.userid = userid;
	}
	public String getOrderno() {
		return orderno;
	}
	public void setOrderno(String orderno) {
		this.orderno = orderno;
	}
	public String getCertfirstimg() {
		return certfirstimg;
	}
	public void setCertfirstimg(String certfirstimg) {
		this.certfirstimg = certfirstimg;
	}
	public String getPartnerid() {
		return partnerid;
	}
	public void setPartnerid(String partnerid) {
		this.partnerid = partnerid;
	}
	public String getSchname() {
		return schname;
	}
	public void setSchname(String schname) {
		this.schname = schname;
	}
	public String getUsername() {
		return username;
	}
	public void setUsername(String username) {
		this.username = username;
	}
	public String getMarried() {
		return married;
	}
	public void setMarried(String married) {
		this.married = married;
	}
	public String getSchcity() {
		return schcity;
	}
	public void setSchcity(String schcity) {
		this.schcity = schcity;
	}
	public String getEmergencymobile() {
		return emergencymobile;
	}
	public void setEmergencymobile(String emergencymobile) {
		this.emergencymobile = emergencymobile;
	}
	public String getIncome() {
		return income;
	}
	public void setIncome(String income) {
		this.income = income;
	}
	public String getMerchanttel() {
		return merchanttel;
	}
	public void setMerchanttel(String merchanttel) {
		this.merchanttel = merchanttel;
	}
	public String getHostname() {
		return hostname;
	}
	public void setHostname(String hostname) {
		this.hostname = hostname;
	}
	public String getHighestlevel() {
		return highestlevel;
	}
	public void setHighestlevel(String highestlevel) {
		this.highestlevel = highestlevel;
	}
	public String getIndustry() {
		return industry;
	}
	public void setIndustry(String industry) {
		this.industry = industry;
	}
	public String getUsermobile() {
		return usermobile;
	}
	public void setUsermobile(String usermobile) {
		this.usermobile = usermobile;
	}
	public String getChannelid() {
		return channelid;
	}
	public void setChannelid(String channelid) {
		this.channelid = channelid;
	}
	public String getBusinesstype() {
		return businesstype;
	}
	public void setBusinesstype(String businesstype) {
		this.businesstype = businesstype;
	}
	public String getApplyamt() {
		return applyamt;
	}
	public void setApplyamt(String applyamt) {
		this.applyamt = applyamt;
	}
	public String getPartnercontantmobile() {
		return partnercontantmobile;
	}
	public void setPartnercontantmobile(String partnercontantmobile) {
		this.partnercontantmobile = partnercontantmobile;
	}
	public String getPartneraccounttype() {
		return partneraccounttype;
	}
	public void setPartneraccounttype(String partneraccounttype) {
		this.partneraccounttype = partneraccounttype;
	}
	public String getMacodes() {
		return macodes;
	}
	public void setMacodes(String macodes) {
		this.macodes = macodes;
	}
	public String getOcrcertno() {
		return ocrcertno;
	}
	public void setOcrcertno(String ocrcertno) {
		this.ocrcertno = ocrcertno;
	}
	public String getEntnature() {
		return entnature;
	}
	public void setEntnature(String entnature) {
		this.entnature = entnature;
	}
	public String getUuid() {
		return uuid;
	}
	public void setUuid(String uuid) {
		this.uuid = uuid;
	}
	public String getCompanyno() {
		return companyno;
	}
	public void setCompanyno(String companyno) {
		this.companyno = companyno;
	}
	public String getBusid() {
		return busid;
	}
	public void setBusid(String busid) {
		this.busid = busid;
	}
	public String getPosition() {
		return position;
	}
	public void setPosition(String position) {
		this.position = position;
	}
	public String getContactmobile() {
		return contactmobile;
	}
	public void setContactmobile(String contactmobile) {
		this.contactmobile = contactmobile;
	}
	public String getGetcontacts() {
		return getcontacts;
	}
	public void setGetcontacts(String getcontacts) {
		this.getcontacts = getcontacts;
	}
	public String getBiztypeenum() {
		return biztypeenum;
	}
	public void setBiztypeenum(String biztypeenum) {
		this.biztypeenum = biztypeenum;
	}
	public String getCertno() {
		return certno;
	}
	public void setCertno(String certno) {
		this.certno = certno;
	}
	public String getModelno() {
		return modelno;
	}
	public void setModelno(String modelno) {
		this.modelno = modelno;
	}
	public String getManames() {
		return manames;
	}
	public void setManames(String manames) {
		this.manames = manames;
	}
	public String getWechatno() {
		return wechatno;
	}
	public void setWechatno(String wechatno) {
		this.wechatno = wechatno;
	}
	public String getDebitbank() {
		return debitbank;
	}
	public void setDebitbank(String debitbank) {
		this.debitbank = debitbank;
	}
	public String getPartnercontantemail() {
		return partnercontantemail;
	}
	public void setPartnercontantemail(String partnercontantemail) {
		this.partnercontantemail = partnercontantemail;
	}
	public String getBranchid() {
		return branchid;
	}
	public void setBranchid(String branchid) {
		this.branchid = branchid;
	}
	public String getQuarters() {
		return quarters;
	}
	public void setQuarters(String quarters) {
		this.quarters = quarters;
	}
	public String getToken() {
		return token;
	}
	public void setToken(String token) {
		this.token = token;
	}
	public String getCooporgno() {
		return cooporgno;
	}
	public void setCooporgno(String cooporgno) {
		this.cooporgno = cooporgno;
	}
	public String getCreditcard() {
		return creditcard;
	}
	public void setCreditcard(String creditcard) {
		this.creditcard = creditcard;
	}
	public String getCredtype() {
		return credtype;
	}
	public void setCredtype(String credtype) {
		this.credtype = credtype;
	}
	public String getTelecode() {
		return telecode;
	}
	public void setTelecode(String telecode) {
		this.telecode = telecode;
	}
	public String getApcrnames() {
		return apcrnames;
	}
	public void setApcrnames(String apcrnames) {
		this.apcrnames = apcrnames;
	}
	public String getHometel() {
		return hometel;
	}
	public void setHometel(String hometel) {
		this.hometel = hometel;
	}
	public String getGiftid() {
		return giftid;
	}
	public void setGiftid(String giftid) {
		this.giftid = giftid;
	}
	public String getSid() {
		return sid;
	}
	public void setSid(String sid) {
		this.sid = sid;
	}
	public String getGraddate() {
		return graddate;
	}
	public void setGraddate(String graddate) {
		this.graddate = graddate;
	}
	public String getSystemcode() {
		return systemcode;
	}
	public void setSystemcode(String systemcode) {
		this.systemcode = systemcode;
	}
	public String getCurcode() {
		return curcode;
	}
	public void setCurcode(String curcode) {
		this.curcode = curcode;
	}
	public String getPartnercontant() {
		return partnercontant;
	}
	public void setPartnercontant(String partnercontant) {
		this.partnercontant = partnercontant;
	}
	public String getMerchantorderexptime() {
		return merchantorderexptime;
	}
	public void setMerchantorderexptime(String merchantorderexptime) {
		this.merchantorderexptime = merchantorderexptime;
	}
	public String getGifttips() {
		return gifttips;
	}
	public void setGifttips(String gifttips) {
		this.gifttips = gifttips;
	}
	public String getGiftfeecode() {
		return giftfeecode;
	}
	public void setGiftfeecode(String giftfeecode) {
		this.giftfeecode = giftfeecode;
	}
	public String getCompanyname() {
		return companyname;
	}
	public void setCompanyname(String companyname) {
		this.companyname = companyname;
	}
	public String getProcess() {
		return process;
	}
	public void setProcess(String process) {
		this.process = process;
	}
	public String getUserapcrcodes() {
		return userapcrcodes;
	}
	public void setUserapcrcodes(String userapcrcodes) {
		this.userapcrcodes = userapcrcodes;
	}
	public String getSchdorm() {
		return schdorm;
	}
	public void setSchdorm(String schdorm) {
		this.schdorm = schdorm;
	}
	public String getCollecttime() {
		return collecttime;
	}
	public void setCollecttime(String collecttime) {
		this.collecttime = collecttime;
	}
	public String getPartneraddr() {
		return partneraddr;
	}
	public void setPartneraddr(String partneraddr) {
		this.partneraddr = partneraddr;
	}
	public String getMerchantname() {
		return merchantname;
	}
	public void setMerchantname(String merchantname) {
		this.merchantname = merchantname;
	}
	public String getProductno() {
		return productno;
	}
	public void setProductno(String productno) {
		this.productno = productno;
	}
	public String getOcrname() {
		return ocrname;
	}
	public void setOcrname(String ocrname) {
		this.ocrname = ocrname;
	}
	public String getFcatschname() {
		return fcatschname;
	}
	public void setFcatschname(String fcatschname) {
		this.fcatschname = fcatschname;
	}
	public String getHostip() {
		return hostip;
	}
	public void setHostip(String hostip) {
		this.hostip = hostip;
	}
	public String getContactrelation() {
		return contactrelation;
	}
	public void setContactrelation(String contactrelation) {
		this.contactrelation = contactrelation;
	}
	public String getAuthpwd() {
		return authpwd;
	}
	public void setAuthpwd(String authpwd) {
		this.authpwd = authpwd;
	}
	public String get_macwifi() {
		return _macwifi;
	}
	public void set_macwifi(String _macwifi) {
		this._macwifi = _macwifi;
	}
	public String getPartnercompany() {
		return partnercompany;
	}
	public void setPartnercompany(String partnercompany) {
		this.partnercompany = partnercompany;
	}
	public String getUserapcrnames() {
		return userapcrnames;
	}
	public void setUserapcrnames(String userapcrnames) {
		this.userapcrnames = userapcrnames;
	}
	public String getUsercoremobile() {
		return usercoremobile;
	}
	public void setUsercoremobile(String usercoremobile) {
		this.usercoremobile = usercoremobile;
	}
	public String get_deviceid() {
		return _deviceid;
	}
	public void set_deviceid(String _deviceid) {
		this._deviceid = _deviceid;
	}
	public String getAuthname() {
		return authname;
	}
	public void setAuthname(String authname) {
		this.authname = authname;
	}
	public String getLbsaddress() {
		return lbsaddress;
	}
	public void setLbsaddress(String lbsaddress) {
		this.lbsaddress = lbsaddress;
	}
	public String getRegisterorg() {
		return registerorg;
	}
	public void setRegisterorg(String registerorg) {
		this.registerorg = registerorg;
	}
	public String getGiftfeetype() {
		return giftfeetype;
	}
	public void setGiftfeetype(String giftfeetype) {
		this.giftfeetype = giftfeetype;
	}
	public String getRequest() {
		return request;
	}
	public void setRequest(String request) {
		this.request = request;
	}
	public String getCreditmode() {
		return creditmode;
	}
	public void setCreditmode(String creditmode) {
		this.creditmode = creditmode;
	}
	public String getBrithday() {
		return brithday;
	}
	public void setBrithday(String brithday) {
		this.brithday = brithday;
	}
	public String getFingerprinttab() {
		return fingerprinttab;
	}
	public void setFingerprinttab(String fingerprinttab) {
		this.fingerprinttab = fingerprinttab;
	}
	public String getIdentitycode() {
		return identitycode;
	}
	public void setIdentitycode(String identitycode) {
		this.identitycode = identitycode;
	}
	public String getPaymode() {
		return paymode;
	}
	public void setPaymode(String paymode) {
		this.paymode = paymode;
	}
	public String getSmscode() {
		return smscode;
	}
	public void setSmscode(String smscode) {
		this.smscode = smscode;
	}
	public String getCredit_productno() {
		return credit_productno;
	}
	public void setCredit_productno(String credit_productno) {
		this.credit_productno = credit_productno;
	}
	public String getGifttype() {
		return gifttype;
	}
	public void setGifttype(String gifttype) {
		this.gifttype = gifttype;
	}
	public String getCpcrcodes() {
		return cpcrcodes;
	}
	public void setCpcrcodes(String cpcrcodes) {
		this.cpcrcodes = cpcrcodes;
	}
	public String getMerchantordertime() {
		return merchantordertime;
	}
	public void setMerchantordertime(String merchantordertime) {
		this.merchantordertime = merchantordertime;
	}
	public String getNation() {
		return nation;
	}
	public void setNation(String nation) {
		this.nation = nation;
	}
	public String getMerchantid() {
		return merchantid;
	}
	public void setMerchantid(String merchantid) {
		this.merchantid = merchantid;
	}
	public String getResitemno() {
		return resitemno;
	}
	public void setResitemno(String resitemno) {
		this.resitemno = resitemno;
	}
	public String getSchyear() {
		return schyear;
	}
	public void setSchyear(String schyear) {
		this.schyear = schyear;
	}
	public String getUserfullife() {
		return userfullife;
	}
	public void setUserfullife(String userfullife) {
		this.userfullife = userfullife;
	}
	public String getPartnername() {
		return partnername;
	}
	public void setPartnername(String partnername) {
		this.partnername = partnername;
	}
	public String getSessionid() {
		return sessionid;
	}
	public void setSessionid(String sessionid) {
		this.sessionid = sessionid;
	}
	public String getSchcitycode() {
		return schcitycode;
	}
	public void setSchcitycode(String schcitycode) {
		this.schcitycode = schcitycode;
	}
	public String getEmail() {
		return email;
	}
	public void setEmail(String email) {
		this.email = email;
	}
	public String getRpflag() {
		return rpflag;
	}
	public void setRpflag(String rpflag) {
		this.rpflag = rpflag;
	}
	public String getApplicant() {
		return applicant;
	}
	public void setApplicant(String applicant) {
		this.applicant = applicant;
	}
	public String getBiztype() {
		return biztype;
	}
	public void setBiztype(String biztype) {
		this.biztype = biztype;
	}
	public String getMerchantno() {
		return merchantno;
	}
	public void setMerchantno(String merchantno) {
		this.merchantno = merchantno;
	}
	public String getDebitcard() {
		return debitcard;
	}
	public void setDebitcard(String debitcard) {
		this.debitcard = debitcard;
	}
	public String getMerchantmobile() {
		return merchantmobile;
	}
	public void setMerchantmobile(String merchantmobile) {
		this.merchantmobile = merchantmobile;
	}
	public String getChannelenum() {
		return channelenum;
	}
	public void setChannelenum(String channelenum) {
		this.channelenum = channelenum;
	}
	public String getPurseid() {
		return purseid;
	}
	public void setPurseid(String purseid) {
		this.purseid = purseid;
	}
	public String getMobile() {
		return mobile;
	}
	public void setMobile(String mobile) {
		this.mobile = mobile;
	}
	public String getIpv4() {
		return ipv4;
	}
	public void setIpv4(String ipv4) {
		this.ipv4 = ipv4;
	}
	public String getSex() {
		return sex;
	}
	public void setSex(String sex) {
		this.sex = sex;
	}
	public String getConsumtype() {
		return consumtype;
	}
	public void setConsumtype(String consumtype) {
		this.consumtype = consumtype;
	}
	public String getDepartment() {
		return department;
	}
	public void setDepartment(String department) {
		this.department = department;
	}
	public String getMerchantorderno() {
		return merchantorderno;
	}
	public void setMerchantorderno(String merchantorderno) {
		this.merchantorderno = merchantorderno;
	}
	public String getEmergencyrelation() {
		return emergencyrelation;
	}
	public void setEmergencyrelation(String emergencyrelation) {
		this.emergencyrelation = emergencyrelation;
	}
	public String getPartneraccounter() {
		return partneraccounter;
	}
	public void setPartneraccounter(String partneraccounter) {
		this.partneraccounter = partneraccounter;
	}
	public String getApcrcodes() {
		return apcrcodes;
	}
	public void setApcrcodes(String apcrcodes) {
		this.apcrcodes = apcrcodes;
	}
	public String getRecommend() {
		return recommend;
	}
	public void setRecommend(String recommend) {
		this.recommend = recommend;
	}
	public String getMerchantregistertime() {
		return merchantregistertime;
	}
	public void setMerchantregistertime(String merchantregistertime) {
		this.merchantregistertime = merchantregistertime;
	}
	public String getGiftno() {
		return giftno;
	}
	public void setGiftno(String giftno) {
		this.giftno = giftno;
	}
	public String getCreditbank() {
		return creditbank;
	}
	public void setCreditbank(String creditbank) {
		this.creditbank = creditbank;
	}
	public String getChanno() {
		return channo;
	}
	public void setChanno(String channo) {
		this.channo = channo;
	}
	public String getCpcrnames() {
		return cpcrnames;
	}
	public void setCpcrnames(String cpcrnames) {
		this.cpcrnames = cpcrnames;
	}
	public String getBizno() {
		return bizno;
	}
	public void setBizno(String bizno) {
		this.bizno = bizno;
	}
	public String getSchroll() {
		return schroll;
	}
	public void setSchroll(String schroll) {
		this.schroll = schroll;
	}
	public String getQq() {
		return qq;
	}
	public void setQq(String qq) {
		this.qq = qq;
	}
	public String getEmergencyname() {
		return emergencyname;
	}
	public void setEmergencyname(String emergencyname) {
		this.emergencyname = emergencyname;
	}
	public String get_macbluetooth() {
		return _macbluetooth;
	}
	public void set_macbluetooth(String _macbluetooth) {
		this._macbluetooth = _macbluetooth;
	}
	public String getLogno() {
		return logno;
	}
	public void setLogno(String logno) {
		this.logno = logno;
	}
	public String getBranchcomp() {
		return branchcomp;
	}
	public void setBranchcomp(String branchcomp) {
		this.branchcomp = branchcomp;
	}
	public String getTermid() {
		return termid;
	}
	public void setTermid(String termid) {
		this.termid = termid;
	}
	public String getCompanyaddress() {
		return companyaddress;
	}
	public void setCompanyaddress(String companyaddress) {
		this.companyaddress = companyaddress;
	}
	public String getMerchantaddress() {
		return merchantaddress;
	}
	public void setMerchantaddress(String merchantaddress) {
		this.merchantaddress = merchantaddress;
	}
	public String getUserno() {
		return userno;
	}
	public void setUserno(String userno) {
		this.userno = userno;
	}
	public String getPartneraccount() {
		return partneraccount;
	}
	public void setPartneraccount(String partneraccount) {
		this.partneraccount = partneraccount;
	}
	public String getMchno() {
		return mchno;
	}
	public void setMchno(String mchno) {
		this.mchno = mchno;
	}
	public String getAddress() {
		return address;
	}
	public void setAddress(String address) {
		this.address = address;
	}
	public String getContactname() {
		return contactname;
	}
	public void setContactname(String contactname) {
		this.contactname = contactname;
	}
	public String getRegistered() {
		return registered;
	}
	public void setRegistered(String registered) {
		this.registered = registered;
	}
	public String getNowphoto() {
		return nowphoto;
	}
	public void setNowphoto(String nowphoto) {
		this.nowphoto = nowphoto;
	}
	public String getConsumapplyid() {
		return consumapplyid;
	}
	public void setConsumapplyid(String consumapplyid) {
		this.consumapplyid = consumapplyid;
	}
	public String getTips() {
		return tips;
	}
	public void setTips(String tips) {
		this.tips = tips;
	}
	public String getCompanytel() {
		return companytel;
	}
	public void setCompanytel(String companytel) {
		this.companytel = companytel;
	}
	public String getWorktime() {
		return worktime;
	}
	public void setWorktime(String worktime) {
		this.worktime = worktime;
	}
	public String getChannelcode() {
		return channelcode;
	}
	public void setChannelcode(String channelcode) {
		this.channelcode = channelcode;
	}
	public String getChannelsource() {
		return channelsource;
	}
	public void setChannelsource(String channelsource) {
		this.channelsource = channelsource;
	}
	public String getAuthenticationflag() {
		return authenticationflag;
	}
	public void setAuthenticationflag(String authenticationflag) {
		this.authenticationflag = authenticationflag;
	}
	public String getChannelmobile() {
		return channelmobile;
	}
	public void setChannelmobile(String channelmobile) {
		this.channelmobile = channelmobile;
	}
	public String getLbs() {
		return lbs;
	}
	public void setLbs(String lbs) {
		this.lbs = lbs;
	}
	public String getLbsx() {
		return lbsx;
	}
	public void setLbsx(String lbsx) {
		this.lbsx = lbsx;
	}
	public String getLbsy() {
		return lbsy;
	}
	public void setLbsy(String lbsy) {
		this.lbsy = lbsy;
	}
	public String getLbsanalyzed() {
		return lbsanalyzed;
	}
	public void setLbsanalyzed(String lbsanalyzed) {
		this.lbsanalyzed = lbsanalyzed;
	}
	public String getLbsprovince() {
		return lbsprovince;
	}
	public void setLbsprovince(String lbsprovince) {
		this.lbsprovince = lbsprovince;
	}
	public String getReceivetimefmt() {
		return receivetimefmt;
	}
	public void setReceivetimefmt(String receivetimefmt) {
		this.receivetimefmt = receivetimefmt;
	}
	public String getCollecttimefmt() {
		return collecttimefmt;
	}
	public void setCollecttimefmt(String collecttimefmt) {
		this.collecttimefmt = collecttimefmt;
	}
	public String getBelongprovince() {
		return belongprovince;
	}
	public void setBelongprovince(String belongprovince) {
		this.belongprovince = belongprovince;
	}
	public String getReceivetime() {
		return receivetime;
	}
	public void setReceivetime(String receivetime) {
		this.receivetime = receivetime;
	}
	 
}
