...
public class ContactCallConsumer {

	private final Logger logger = LoggerFactory.getLogger(getClass());
	private ConsumerConnector consumer;
	public static void main(String[] args) {
		System.out.println("开始启动通讯录通话记录kafka接收消息.");
		ContactCallConsumer customers=new ContactCallConsumer();
		customers.consume();
	...
	private void initial() {
    	System.out.println("初始化kafka配置 zookeeper.connect:"+ConstantsConfig.kafkaZookeeperConnect+"   group.id:"+ConstantsConfig.kafkaGroup+"   kafkaTopic:"+ConstantsConfig.kafkaCallContactTopic);
...
//        props.put("auto.offset.reset", "smallest");
	...
	private List<String> getPropertyList() {
		...
		try {
			...
			System.err.println("ContactCallConsumer 获取属性失败 error："+e.getMessage());
		}
		return propertyList;
...
//	        topicCountMap.put(kafkaTopic, new Integer(1));
	        topicCountMap.put(ConstantsConfig.kafkaCallContactTopic, new Integer(1));
	        StringDecoder keyDecoder = new StringDecoder(new VerifiableProperties());
	        StringDecoder valueDecoder = new StringDecoder(new VerifiableProperties());
	        Map<String, List<KafkaStream<String, String>>> consumerMap = 
	                consumer.createMessageStreams(topicCountMap,keyDecoder,valueDecoder);
	        KafkaStream<String, String> stream = consumerMap.get(ConstantsConfig.kafkaCallContactTopic).get(0);
	        ...
	        while (true) {
	        	try {
					...
					System.err.println("ContactCallConsumer接收到的消息处理失败 ERROR:"+e.getMessage());
				}
				Thread.sleep(5000);
			}
