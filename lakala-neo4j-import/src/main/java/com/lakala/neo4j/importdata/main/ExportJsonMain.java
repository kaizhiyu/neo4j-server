...
public class ExportJsonMain {
	...
	public static void main(String[] args) {
		...
		StringBuilder sbApplysub=getApplyLoanInfoProps("AX20170205150226082350");
	...
	public static StringBuilder getApplyLoanInfoProps(String orderno){
		...
		IMEILoanApplyNeo4j loanApplyNeo4j  = JSON.parseObject(collection.selectNode, IMEILoanApplyNeo4j.class);
		if(!idMap.containsKey(loanApplyNeo4j.getId()))
		{
			idMap.put(loanApplyNeo4j.getId(),loanApplyNeo4j.getOrderno());
		}
		
		for(String modelstr:  collection.modelList){
			IMEIModelNeo4j modelNeo4j  = JSON.parseObject(modelstr, IMEIModelNeo4j.class);
			if(!idMap.containsKey(modelNeo4j.getId()))
			{
				idMap.put(modelNeo4j.getId(),modelNeo4j.getContent());
		...
		for(String relationstr: collection.relationList){
			...
			if(!eList.contains(relationNeo4j.getId())){
				...
				relationE.setSource(idMap.get(relationE.getSource()));
				relationE.setTarget(idMap.get(relationE.getTarget()));
...
//			}
//		}
		IMEIModelNeo4j modelNeo4j  = JSON.parseObject(collection.selectNode, IMEIModelNeo4j.class);
		if(!idMap.containsKey(modelNeo4j.getId()))
		{
			idMap.put(modelNeo4j.getId(),modelNeo4j.getContent());
		}
		...
		for(String modelstr:  collection.modelList){
			...
			if(!idMap.containsKey(loanApplyNeo4j.getId()))
			{
				...
				applyInfoV.setModelname("ApplyInfo");
	...
	public static StringBuilder getApplyLoanInfo(String orderNo){
		...
		sbStringBuilder.append("[");
		StringBuilder sbmodelstr=new StringBuilder();//实体对应的json集合
		Map<String,String> idMap=new HashMap<String,String>();//所有id映射对应的name值，如果是进件那么对应的是orderno，其他的实体对应的是content
			...
			for(String modelstr:  collection2.modelList){
				...
				if(modelstr.contains(ModelType.APPLYINFO.getModelName())){//进件实体
					...
					if(!idMap.containsKey(loanApplyNeo4j.getId()))
					{
						idMap.put(loanApplyNeo4j.getId(),loanApplyNeo4j.getOrderno());
						IMEIApplyInfoV applyInfoV=Neo4jModelToIMEIModel.ApplyNeo4jToApplyV(loanApplyNeo4j);
						if(applyInfoV.getOrderno().equals(orderNo))
						{
						  applyInfoV.setSymbolSize(60);
						}
						applyInfoV.setModelname("ApplyInfo");
						String json = JSONObject.toJSON(applyInfoV).toString();
						sbmodelstr.append(json);
